...
var Server = function () {
	...
	_createClass(Server, [{
		key: 'registerEvents',
		value: function registerEvents(socket) {
			//this.registerPassThroughEvent(Events.CHAT, socket)
			this.registerStorageEvent(_events2.default.RESULT, socket);
			this.registerRepositoryEvent(_events2.default.CONFIGURED, socket);
		}
	}, {
		key: 'setReady',
		...
		key: 'registerRepositoryEvent',
		value: function registerRepositoryEvent(event, socket) {
		...
		key: 'registerPassThroughEvent',
		value: function registerPassThroughEvent(event, socket) {
			var _this3 = this;
		...
		key: 'registerStorageEvent',
		value: function registerStorageEvent(event, socket) {
			var _this4 = this;

			socket.on(event, function (message) {
				var json = _common2.default.toJson(message);
				console.info(_common2.default.now() + ' Storage event ' + event, json);
				_this4.socketServer.sockets.in(_agentRepository2.default.STORAGE_AGENT_TAG).emit(event, message);
			});
		}
	}]);
